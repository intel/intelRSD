...
/*!
 * @brief Discovers processor using SMBIOS parser
 *
 * @param[in] smbios Pointer to SMBIOS parser
 * @param[in] system Reference to system model to be populated with processor data
 *
 * @return Tuple that tells the caller discovery status and health status to be set on the resource
 * */
std::tuple<bool, agent_framework::model::enums::Health> discover_processor(::smbios::parser::SmbiosParser::Ptr smbios,
