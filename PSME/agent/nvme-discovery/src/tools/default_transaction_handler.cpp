...
void DefaultTransactionHandler::add_handler_pair(BaseTransactionHandler::func_pair functions) {
    ...
    std::for_each(m_handler.rbegin() + (m_handler.size() - executed), m_handler.rend(),
        [](const TransactionContainer::value_type& pair) {
            try {
                ...
                log_warning("nvme-discovery-agent", "Failed during rollback: " << e.what());
            }
        });
}
