...
void get_collection(GetCollection::ContextPtr, const GetCollection::Request& request,
    ...
    log_debug("nvme-discovery-agent", "Getting collection of " << name);
    if (process_resource_collections<Manager,
        ...
        THROW(agent_framework::exceptions::InvalidUuid, "nvme-discovery-agent",
              "Component not found - invalid UUID: " + uuid);
    }

    log_debug("nvme-discovery-agent", "Getting " << name << " collection successful.");
}
