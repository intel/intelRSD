...
void set_component_attributes(SetComponentAttributes::ContextPtr context, const SetComponentAttributes::Request& req,
    ...
    log_debug("nvme-discovery-agent", "Executing setComponentAttributes.");
    const auto& uuid = req.get_component();
    const auto& attributes = req.get_attributes();

    if (attributes.empty()) {
        log_debug("nvme-discovery-agent", "Nothing has been changed (empty request).");
        return;
    }

    if (get_manager<Chassis>().entry_exists(uuid)) {
        command::set_chassis_attributes(context, uuid, attributes, res);
    } else if (resource_is_unchangeable(uuid)) {
        THROW(InvalidValue, "nvme-discovery-agent", "Operation not available for this component.");
    }
    else {
        THROW(InvalidUuid, "nvme-discovery-agent", "Component not found.");
    }
    log_debug("nvme-discovery-agent", "setComponentAttributes finished successfully.");
}
