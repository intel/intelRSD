...
void set_asset_tag(const Uuid& uuid, const json::Json& value) {
    ...
    log_debug("fpgaof-agent", "Setting assetTag for chassis " << uuid);
    {
    ...
    log_info("fpgaof-agent", "Attribute 'assetTag' has been set to: " << value.dump());
}
...
void set_chassis_attributes(AgentContext::SPtr, const Uuid& uuid, const attribute::Attributes& attributes,
    ...
    log_info("fpgaof-agent", "Processing SetChassisAttributes [UUID = " << uuid << "]");
    CommonValidator::validate_set_chassis_attributes(attributes);

    const auto attribute_names = attributes.get_names();
    if (attribute_names.empty()) {
        log_debug("fpgaof-agent", "setComponentAttributes: nothing has been changed (empty request).");
        return;
    }

    for (const auto& name : attribute_names) {
        const auto& value = attributes.get_value(name);
        log_debug("fpgaof-agent", "Attribute name: " << name);
        ...
        try {
            if (literals::Chassis::ASSET_TAG == name) {
                ...
                THROW(UnsupportedField, "fpgaof-agent", "Setting attribute is not supported.", name, value);
            }
        }
