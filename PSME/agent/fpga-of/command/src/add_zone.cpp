...
void store_to_database(const Uuid& zone, const AddZone::Request& req) {
    FabricEntity(get_manager<Fabric>().get_keys().front()).append(ZONES_PROPERTY, zone);
    ZoneEntity zone_db(zone);
    const auto& endpoints = req.get_endpoints();
    for_each(endpoints.begin(), endpoints.end(),
             [&zone_db](const Uuid& endpoint) { zone_db.append(ZONE_ENDPOINTS_PROPERTY, endpoint); });
}
...
void add_zone(AddZone::ContextPtr ctx, const AddZone::Request& req, AddZone::Response& rsp) {
    ...
    if (!initiators.empty() && !targets.empty()) {
        auto host_uuid = attribute::Identifier::get_uuid(initiators.front());
        for (const auto& target : targets) {
            auto device_uuid = target.get_connected_entities().front().get_entity().value();
    ...
    log_info("fpgaof-agent", "Added zone with UUID '" + zone.get_uuid() + "'");
}
}

REGISTER_FPGAOF_COMMAND(AddZone, ::add_zone);
