...
void get_network_interface_info(GetNetworkInterfaceInfo::ContextPtr,
                                const GetNetworkInterfaceInfo::Request& request,
                                GetNetworkInterfaceInfo::Response& response) {
    log_debug("fpgaof-agent", "Getting network interface info.");
    response = module::get_manager<model::NetworkInterface>().get_entry(request.get_uuid());
}

}

REGISTER_FPGAOF_COMMAND(GetNetworkInterfaceInfo, ::get_network_interface_info);
