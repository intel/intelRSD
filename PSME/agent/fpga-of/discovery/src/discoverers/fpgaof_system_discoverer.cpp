...
agent_framework::model::System FpgaofSystemDiscoverer::discover(const Uuid& parent_uuid,
                                                                const Uuid& chassis_uuid) {
    auto system = FpgaofSystemBuilder::build_virtual(parent_uuid, chassis_uuid);

    try {
        auto guid_file = m_sysfs->get_file(GUID_PATH);
        agent_framework::discovery::SystemBuilder::update_with_guid(system, guid_file);
    }
