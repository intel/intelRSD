/*!
 * @brief Definition of FpgaKeyGenerator class.
 *
...
protected:
    ...
    static std::string generate_key_base(const T& resource) {
        return m_agent_id + resource.get_component().to_string();
    }
...
template<>
std::string FpgaKeyGenerator::generate_key(const agent_framework::model::Metric& metric, const agent_framework::model::Resource& resource);

template<>
std::string FpgaKeyGenerator::generate_key(const agent_framework::model::MetricDefinition& metric_def);
