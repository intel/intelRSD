...
void set_component_attributes(SetComponentAttributes::ContextPtr context,
    ...
    log_info("spdk-agent", "Trying to execute SetComponentAttributes method.");
    const auto& uuid = request.get_component();
    const auto& attributes = request.get_attributes();

    if (get_manager<Volume>().entry_exists(uuid)) {
        command::set_volume_attributes(context, uuid, attributes, response);
    }
    ...
    log_info("spdk-agent", "setComponentAttributes finished successfully.");
}

}

REGISTER_SPDK_COMMAND(SetComponentAttributes, ::set_component_attributes);
