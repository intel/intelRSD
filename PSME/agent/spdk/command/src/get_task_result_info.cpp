 ...
 *
 * @brief Registers GetTaskResultInfo command
...
void get_task_result_info(GetTaskResultInfo::ContextPtr,
    ...
    if (!task_result_manager->task_exists(task.get_uuid())) {
        THROW(agent_framework::exceptions::InvalidUuid, "spdk-agent", "Could not find requested task.");
    }
...
REGISTER_SPDK_COMMAND(GetTaskResultInfo, ::get_task_result_info);
