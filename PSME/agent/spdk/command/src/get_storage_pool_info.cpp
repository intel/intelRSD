...
void get_storage_pool_info(GetStoragePoolInfo::ContextPtr,
                           const GetStoragePoolInfo::Request& request,
                           GetStoragePoolInfo::Response& response) {

    log_debug("spdk-agent", "Getting storage pool info.");
    response = get_manager<StoragePool>().get_entry(request.get_uuid());
}
}

REGISTER_SPDK_COMMAND(GetStoragePoolInfo, ::get_storage_pool_info);
