...
void EthernetInterfaceBuilder::update_with_net(model::NetworkInterface& interface,
    ...
    status.set_state(net_interface.is_up() ?
                     model::enums::State::Enabled : model::enums::State::Disabled);
    ...
    for(std::size_t index = 0; index < addresses.size(); ++index) {
        ...
        if (ip_address.get_address_family() == net::AddressFamily::IPv4) {
            ...
            log_warning("storage-agent", "Unsupported address family: "
    ...
    log_debug("psme-storage", "MAC address for interface "
