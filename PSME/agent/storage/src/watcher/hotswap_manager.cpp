...
bool compare_drives(const model::Drive& lhs, const model::Drive& rhs) {
    ...
    return (
        ...
        && (Identifier::get_system_path(lhs) == Identifier::get_system_path(rhs))
    );
}
...
void HotswapManager::hotswap_discover_hard_drives() {
    ...
    Drives drives_detected{};
    /* Get list of hard drives on a system */
    m_discoverer->discover_drives(drives_detected, chassis_uuid);
...
void HotswapManager::add_drive(model::Drive& detected_drive, const Uuid& storage_service_uuid) {
    ...
    DriveBuilder::update_identifier_with_uuid(detected_drive);
    ...
    for (const auto& pool_uuid : pool_uuids) {
        auto pool = module::get_manager<model::StoragePool>().get_entry_reference(pool_uuid);
        auto count_before_discovery = pool->get_capacity_sources().size();
        m_discoverer->discover_capacity_sources(pool.get_raw_ref());
        auto count_after_discovery = pool->get_capacity_sources().size();
        if (count_before_discovery != count_after_discovery) {
            // Pool was changed -> hard drives were attached
            eventing::send_update(pool.get_raw_ref());
        }
    }

    /* Log successful adding */
    log_debug("hot-swap", "Added new drive: " << model::attribute::Identifier::get_system_path(detected_drive));
}
...
void HotswapManager::remove_drive(const model::Drive& removed_drive) {
    ...
    log_debug("hot-swap",
              "Removed drive [" << removed_drive.get_uuid() << "]: "
                                << model::attribute::Identifier::get_system_path(removed_drive));
}
