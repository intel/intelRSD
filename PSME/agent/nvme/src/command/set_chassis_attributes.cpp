...
void set_asset_tag(const Uuid& uuid, const json::Json& value) {
    ...
    log_debug("nvme-agent", "Setting assetTag");
    ...
    log_debug("nvme-agent", "assetTag has been set to: " << value.dump());
}
...
void agent::nvme::command::set_chassis_attributes(agent::nvme::SetComponentAttributes::ContextPtr /*context*/,
    ...
    log_debug("nvme-agent", "Executing setChassisAttributes command");
    log_debug("nvme-agent", "Processing Chassis [UUID = " << uuid << "]");
    ...
    for (const auto& attribute_name : attributes.get_names()) {
        ...
        if (literals::Chassis::ASSET_TAG == attribute_name) {
            try {
                ...
                THROW(NvmeError, "nvme-agent", std::string{"Error while setting chassis attributes: "} + e.what());
            }
        }
        else {
