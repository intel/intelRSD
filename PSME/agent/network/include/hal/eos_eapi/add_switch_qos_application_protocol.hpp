/*!
 * @brief Add QoS application protocol command class declaration.
 *
 ...
 *
 * @file add_switch_qos_application_protocol.hpp
 */
...
/*!
 * @brief Class representing eAPI Add QoS Application Protocol command
 */
class AddSwitchQosApplicationProtocol : public EapiCommand {
public:
     ...
     */
    AddSwitchQosApplicationProtocol(ProtocolId protocol, std::uint32_t port, std::uint32_t priority)
        : EapiCommand{EAPI_COMMAND_JSON},
          m_protocol{protocol}, m_port{port}, m_priority{priority} { }

    /*! Destructor */
    virtual ~AddSwitchQosApplicationProtocol();
