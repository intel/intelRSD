...
namespace {
    inline void create_static_mac(const string& port, const string& address,
        ...
        catch (const netlink_base::NetlinkInvalidParamsException& e) {
            log_error(GET_LOGGER("network-agent"), e.what());
            THROW(agent_framework::exceptions::InvalidParameters, "network-agent",
                  string("Failed to create Static MAC on port ") + port + string(": invalid parameters given"));
        }
        catch (const runtime_error& e) {
            log_error(GET_LOGGER("network-agent"), e.what());
            THROW(agent_framework::exceptions::Fm10000Error, "fm10000",
                  string("Failed to create Static MAC on port ") + port + string(": ") + e.what());
        }
    }
    ...
    void add_port_static_mac(const AddPortStaticMac::Request& request,
        ...
        if (mac_vlan_exists_on_port(port.get_uuid(), mac_address, vlan_id,
                                    nullptr)) {
            THROW(agent_framework::exceptions::InvalidParameters, "fm10000",
