...
void add_acl(const AddAcl::Request& request, AddAcl::Response& response) {
    ...
    acl.set_name(acl.get_uuid());
    const std::string acl_uuid = acl.get_uuid();
    ...
    const std::string acl_persistent_uuid = NetworkTreeStabilizer().stabilize_acl(acl_uuid);

    response.set_acl(acl_persistent_uuid);
}
}
