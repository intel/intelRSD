...
void get_neighbor_collections(const GetCollection::Request& request,
                              GetCollection::Response& response) {
    auto network_components = NetworkComponents::get_instance();
    auto& remote_switch_manager = network_components->get_remote_switch_manager();
    try {
        RemoteSwitchInfo info;
        // get neighbors of a local switch by giving an empty switch id
        info.load_neighbor_switches_id(request.get_uuid());
        ...
        log_warning(GET_LOGGER("fm10000"), "Getting neighbor switches failed.");
...
void get_static_macs_collection(const GetCollection::Request& request,
    ...
    //verify port UUID
    const string& port_uuid = request.get_uuid();
...
void get_collection(const GetCollection::Request& request, GetCollection::Response& response) {
    ...
    if (vlans.to_string() == request.get_name()) {
        ...
        THROW(agent_framework::exceptions::InvalidCollection, "fm10000",
              "Unknown collection requested " + request.get_name());
    }
    log_debug(GET_LOGGER("fm10000"), "Getting collection \"" + request.get_name() + "\" is successfully finished.");
}
}
