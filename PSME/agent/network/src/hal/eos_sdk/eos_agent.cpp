...
namespace {
    sdk _sdk{};
    unordered_map<string, intf_id_t> _port_name_map{};

    void store_port_id(const string& name, const intf_id_t& id) {
        _port_name_map[name] = id;
    }

    const intf_id_t& get_port_id(const string& name) {
...
void EosAgent::on_initialized() {
    log_debug(GET_LOGGER("network-agent"), "Eos Agent initialized");
    m_initialized = true;
    m_cv.notify_all();
}
...
vector<string> EosAgent::get_port_list() const {
    vector<string> ret{};
    if (m_initialized) {
        sdk_scoped_lock ssl;
...
bool EosAgent::is_admin_enabled(const string& intf_name) const {
    sdk_scoped_lock ssl;
    auto* mgr = get_intf_mgr();
...
bool EosAgent::is_operational_up(const string& intf_name) const {
    sdk_scoped_lock ssl;
    auto* mgr = get_intf_mgr();
    auto intf_id = ::get_port_id(intf_name);
...
void EosAgent::set_admin_enable(const string& intf_name, const bool enable) {
    sdk_scoped_lock ssl;
    auto* mgr = get_intf_mgr();
    auto intf_id = ::get_port_id(intf_name);
    try {
        ...
        THROW(NetworkError, "network-agent",
              string("Setting administrative state on port ") + intf_name +
              " failed: " + e.what());
    }
}

uint32_t EosAgent::get_speed(const string& intf_name) {
    sdk_scoped_lock ssl;
...
void EosAgent::set_default_vlan(const std::string& intf_name, uint16_t vid) {
    ...
    try {
        ...
        THROW(NetworkError, "network-agent",
              string("Setting default VLAN to ") + to_string(int(vid)) +
    ...
    if (SWITCHPORT_MODE_TRUNK == mgr->switchport_mode(intf_id)) {
        try {
            ...
            log_error(GET_LOGGER("network-agent"),
                      string("Unable to remove VLAN ") +
                      to_string(int(old_default_vlan)) +
...
void EosAgent::delete_tagged_vlan(const string& intf_name, uint16_t vlan_id) {
    sdk_scoped_lock ssl;
    auto* mgr = get_eth_intf_mgr();
    auto intf_id = ::get_port_id(intf_name);
    if (SWITCHPORT_MODE_TRUNK == mgr->switchport_mode(intf_id)) {
