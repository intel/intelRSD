...
using TestMethod = agent_framework::command::Command<TestMethodRequest, TestMethodResponse>;

REGISTER_COMMAND(TestMethod,
    [] (const TestMethod::Request&, TestMethod::Response& rsp) {
        rsp.text = "Test method response!";
    }
);
...
using CommandServer = agent_framework::command::CommandServer;

int main() {

    std::shared_ptr<HttpServerConnector> connector(new HttpServerConnector(SERVER_PORT));
    std::shared_ptr<CommandServer> server(new CommandServer(connector));
    server->add(agent_framework::command::Registry::get_instance()->get_commands());
