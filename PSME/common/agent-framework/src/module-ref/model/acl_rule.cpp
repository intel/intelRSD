...
AclRule::AclRule(const std::string& parent_uuid) :
    Resource{parent_uuid, enums::Component::Acl} {}

AclRule::~AclRule() {}

Json::Value AclRule::to_json() const {
    Json::Value value;
    ...
    value[literals::AclRule::DESTINATION_MAC] =
        get_destination_mac().to_json();
    value[literals::AclRule::SOURCE_L4_PORT] = get_source_port().to_json();
    value[literals::AclRule::DESTINATION_L4_PORT] =
        get_destination_port().to_json();
    value[literals::AclRule::PROTOCOL] = get_protocol();
    value[literals::AclRule::OEM] = get_oem().to_json();
    return value;
}

AclRule AclRule::from_json(const Json::Value& json) {
    AclRule rule;
    ...
    rule.set_forward_mirror_port(
        json[literals::AclRule::FORWARD_MIRROR_PORT]);
    rule.set_mirrored_ports(MirroredPorts::from_json(
        json[literals::AclRule::MIRRORED_PORTS]));
    rule.set_mirror_type(json[literals::AclRule::MIRROR_TYPE]);
    rule.set_vlan_id(VlanId::from_json(json[literals::AclRule::VLAN_ID]));
    rule.set_source_ip(Ip::from_json(
        json[literals::AclRule::SOURCE_IP]));
    rule.set_destination_ip(Ip::from_json(
        json[literals::AclRule::DESTINATION_IP]));
    rule.set_source_mac(Mac::from_json(json[literals::AclRule::SOURCE_MAC]));
    rule.set_destination_mac(Mac::from_json(
        json[literals::AclRule::DESTINATION_MAC]));
    rule.set_source_port(Port::from_json(
        json[literals::AclRule::SOURCE_L4_PORT]));
    rule.set_destination_port(Port::from_json(
        json[literals::AclRule::DESTINATION_L4_PORT]));
    rule.set_protocol(json[literals::AclRule::PROTOCOL]);
    rule.set_oem(attribute::Oem::from_json(json[literals::AclRule::OEM]));
    rule.set_resource_hash(compute_hash(json));
