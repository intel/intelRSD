 ...
 * */
 #define REGISTER_COMMAND(CMD_HANDLER, CMD_FUNC)                              \
     ...
     bool dummy_##CMD_HANDLER::m_dummy =                                      \
         agent_framework::command_ref::Registry::get_instance()->             \
...
public:
    ...
    using Commands = std::vector<std::unique_ptr<CommandBase>>;
    ...
    bool add_handler(typename T::HandlerFunc command_handler) {
        ...
        m_commands.emplace_back(std::unique_ptr<CommandBase>(new T(command_handler)));
        return true;
    }
