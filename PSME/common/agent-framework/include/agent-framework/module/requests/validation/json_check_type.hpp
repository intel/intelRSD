...
namespace validation {
    ...
    /*!
     * @brief Check if JSON value is string type and has no whitespace charaters
     *
     * @param[in] value JSON value to check
     * @param[in] parameter_name Name of JSON field
     * @param[in] logger_name Logger name
    ...
    void check_string_no_whitespace(const Json::Value& value,
                                    ...
                                    const char* logger_name);
    /*!
     * @brief Check if JSON value is null or string type and has no whitespace charaters
     *
     * @param[in] value JSON value to check
     * @param[in] parameter_name Name of JSON field
     * @param[in] logger_name Logger name
    ...
    /*!
    * @brief Check if string has no whitespace charaters
    *
    ...
    * */
     ...
     * */
    template <typename E>
    void check_enum(const Json::Value& value,
                    const std::string& parameter_name,
                    const char* logger_name) {
        check_string(value, parameter_name, logger_name);
        const auto& string_value = value.asString();
        try {
            E::from_string(string_value);
        }
        catch (...) {
            log_error(GET_LOGGER(logger_name),
                "Invalid value for " << parameter_name << " attribute: '" << string_value << "'.");
            throw agent_framework::exceptions::InvalidValue("Invalid attribute value: '" + string_value + "'.");
        }
    }
     ...
     * */
    template <typename E>
    void check_nullable_enum(const Json::Value& value,
                    const std::string& parameter_name,
                    const char* logger_name) {
        if (!value.isNull()) {
            check_string(value, parameter_name, logger_name);
