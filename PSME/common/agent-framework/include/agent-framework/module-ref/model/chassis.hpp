...
#ifndef AGENT_FRAMEWORK_MODULE_MODEL_CHASSIS_HPP
#define AGENT_FRAMEWORK_MODULE_MODEL_CHASSIS_HPP
...
#include "agent-framework/module-ref/enum/chassis.hpp"
...
public:
    ...
    explicit Chassis(const std::string& parent_uuid = {});
    ~Chassis();
    ...
    /*!
     * @brief Set location offset size
     * @param[in] size location size
     * */
    void set_location_offset_size(const std::uint8_t& size) {
        m_location_offset_size = size;
    }

    /*!
     * @brief Get location offset size
     * @return Location offset size
     * */
    const std::uint8_t& get_location_offset_size() const {
        return m_location_offset_size;
    }
    ...
    void set_parent_id(const std::string& parent_id) {
        if (!parent_id.empty()) {
            m_parent_id =
                static_cast<std::uint32_t>(std::stoul(parent_id, nullptr, 10));
        }
    }
    ...
    const std::string get_parent_id() const {
        return std::string{std::to_string(m_parent_id)};
    }

    /*!
     * @brief Set parent id from uint
     * @param[in] parent_id Parent id
     * */
    void set_parent_id_as_uint(const std::uint32_t parent_id) {
        m_parent_id = parent_id;
    }

    /*!
     * @brief Get Parent id as uint
     * @return Parent id
     * */
    uint32_t get_parent_id_as_uint() const {
        return m_parent_id;
    }
     ...
     * @param[in] size location size
     * */
    void set_parent_id_size(const OptionalField<std::uint8_t>& size) {
        m_parent_id_size = size;
    }
     ...
     * @return Location offset size
     * */
    const OptionalField<uint8_t> get_parent_id_size() const {
        return m_parent_id_size;
    }
...
private:
    ...
    //Non-GAMI fields
    std::uint8_t m_location_offset_size{};
    std::uint8_t m_parent_id_size{};
    // GAMI specifies it to be string, it is more practical to keep it as uint
    // use set/get_parent_id_as_uint to access it as int
...
#endif /* AGENT_FRAMEWORK_MODULE_MODEL_CHASSIS_HPP */
