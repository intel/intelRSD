/*!
 * @copyright
 * Copyright (c) 2015-2016 Intel Corporation
 *
 * @copyright
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * @copyright
 *    http://www.apache.org/licenses/LICENSE-2.0
 *
 * @copyright
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *
 *
 * @file status.hpp
 * @brief Status
 * */

#ifndef AGENT_FRAMEWORK_MODULE_MODEL_ATTRIBUTES_STATUS_HPP
#define AGENT_FRAMEWORK_MODULE_MODEL_ATTRIBUTES_STATUS_HPP

#include "agent-framework/module-ref/enum/common.hpp"
#include "agent-framework/module-ref/utils/utils.hpp"
#include <string>

namespace Json { class Value; }

namespace agent_framework {
namespace model {
namespace attribute {


/*! Storage  status */
class Status {
public:

    explicit Status();

    Status(const enums::State state, const enums::Health health);

    Status(const std::string& state, const std::string& health);

    Status(const bool ok);

    ~Status();

    /*! Enable copy */
    Status(const Status&) = default;
    Status& operator=(const Status&) = default;
    Status(Status&&) = default;
    Status& operator=(Status&&) = default;

    /*!
     * Gets health
     * */
    const enums::State& get_state() const { return m_state; }

    void set_state(const enums::State& state) {
        m_state = state;
    }

    /*!
     * Gets health
     * */
    const enums::Health& get_health() const { return m_health; }

    void set_health(const enums::Health& health) {
        m_health = health;
    }

    /*
     * Converts this object to json representation
     * @return Json representation of this object
     * */
    Json::Value to_json() const;

    /*!
     * @brief construct an object of class Status from JSON
     *
     * @param json the Json::Value deserialized to object
     *
     * @return the newly constructed Status object
     */
    static Status from_json(const Json::Value& json);

private:
    enums::State m_state{enums::State::Disabled};
    enums::Health m_health{enums::Health::OK};
};

}
}
}

#endif /* AGENT_FRAMEWORK_MODULE_MODEL_ATTRIBUTES_STATUS_HPP */
