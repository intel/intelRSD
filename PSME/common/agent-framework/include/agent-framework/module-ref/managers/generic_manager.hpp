...
public:
    enum class UpdateStatus { NoUpdate, Updated, Added };
    using value_type = T;
    ...
    void add_entry(U&& entry_r) {
        ...
        if (m_manager_data.end() != it) {
            THROW(::agent_framework::exceptions::InvalidUuid, "rest",
    ...
    UpdateStatus add_or_update_entry(U&& entry_r) {
        ...
        if (m_manager_data.end() != it) {
            ...
            if (it->get_resource_hash() != entry.get_resource_hash())
    ...
    agent_framework::generic::ObjReference<T, std::recursive_mutex>
    get_entry_reference(const std::string& uuid) {
        std::lock_guard<std::recursive_mutex> lock{m_mutex};
        ...
        if (m_manager_data.end() != it) {
            return agent_framework::generic::ObjReference
                                        <T, std::recursive_mutex>(*it, m_mutex);
        }
        THROW(::agent_framework::exceptions::InvalidUuid, "rest",
              std::string(T::get_collection_name().to_string()) +
                      " [UUID = '" + uuid + "'] not found.");
    ...
    void remove_by_parent(const std::string& uuid) {
        ...
        if (n != 0) {
            log_info(GET_LOGGER("rest"), "Removed " << n << " " << T::get_collection_name().to_string() << ", parent " << uuid);
        }
    }
     ...
     *
     * @param filter a std::fuction, eg. a lambda, serving as a filter
     *
     ...
     */
    const std::string& rest_id_to_uuid(std::uint64_t id, const std::string& parent_uuid = {}) {
    ...
    void clean_resources_for_agent(const std::string& gami_id) {
        ...
        if (n != 0) {
            log_info(GET_LOGGER("rest"), "Removed " << n << " " << T::get_collection_name().to_string() << ", agent " << gami_id);
        }
    }
...
private:
    ...
    typename ManagerDataVec::iterator
    find_entry(const std::string& uuid) {
        auto it = std::find_if(m_manager_data.begin(),
                               m_manager_data.end(),
                               [&uuid](const T& entry) {
                                   return entry.get_uuid() == uuid;
     ...
     */
    const std::string& find_uuid_by_id(std::uint64_t id) {
        std::lock_guard<std::recursive_mutex> lock{m_mutex};
        for (const auto& entry : m_manager_data) {
            if (entry.get_id() == id) {
        ...
        const auto& message = std::string("Could not find ") +
                T::get_collection_name().to_string() +
     ...
     * @return object's uuid
     */
    const std::string& find_uuid_by_id_and_parent(std::uint64_t id, const std::string& parent_uuid) {
        std::lock_guard<std::recursive_mutex> lock{m_mutex};
        for (const auto& entry : m_manager_data) {
        ...
        const auto& message = std::string("Could not find ") +
                T::get_collection_name().to_string() + " with id: " +
                std::to_string(id) + ".";
        log_warning(GET_LOGGER("rpc"), message + " Parent UUID = " +
                                       parent_uuid + ".");
        throw agent_framework::exceptions::NotFound(message);
