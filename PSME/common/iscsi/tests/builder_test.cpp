...
protected:
    virtual void SetUp() {
        m_context.get_network_device_function = [this]() -> fixture::NetworkDeviceFunction {return m_net_dev;};
    }

    virtual void TearDown() {
    }

    DeviceFunctionContext m_context;
...
TEST_F(IscsiMdrBuilderTest, IterateOverInitiators) {
    ...
    const auto& boot = m_context.get_network_device_function().get_iscsi_boot();
    EXPECT_EQ(m_context.get_network_device_function().get_mac_address(), mdr::print_mac(init.data.nic_mac_address));
