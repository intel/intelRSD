...
TEST(NvmeCommandsTest, GetLogPage) {
    ...
    ASSERT_EQ(cmd.get_data_ref().cmd.namespace_id, 123);
    ASSERT_EQ(cmd.get_data_ref().cmd.get_log_page.log_page_id, uint8_t(LogPageId::CommandsAndEffects));
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_smart_log()));
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_firmware_log()));
    static constexpr unsigned EXPECTED_SIZE = 512;
    static constexpr unsigned EXPECTED_DWORDS = EXPECTED_SIZE / 4 - 1;
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.data_length, EXPECTED_SIZE);
    ASSERT_EQ(cmd.get_data_ref().cmd.get_log_page.num_of_dwords, EXPECTED_DWORDS);
}
...
TEST(NvmeCommandsTest, NamespaceAttachment) {
    ...
    const ControllerIdList* cil = reinterpret_cast<const ControllerIdList*>(cmd.get_data_ref().cmd.data_pointer.data.address);
...
TEST(NvmeCommandsTest, NamespaceManagementDelete) {
    ...
    ASSERT_EQ(cmd.get_data_ref().cmd.namespace_management.sel, uint8_t(NamespaceManagementSel::Delete));
    static constexpr unsigned EXPECTED_SIZE = 0;
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.data_length, EXPECTED_SIZE);
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, 0);
...
TEST(NvmeCommandsTest, NamespaceManagementCreate) {
    ...
    ASSERT_EQ(cmd.get_type(), NvmeCommandType::AdminCommand);
    ASSERT_EQ(cmd.get_data_ref().cmd.opcode, uint8_t(AdminCommandOpcode::NamespaceManagement));
    ASSERT_EQ(cmd.get_data_ref().cmd.namespace_management.sel, uint8_t(NamespaceManagementSel::Create));
    static constexpr unsigned EXPECTED_SIZE = 4096;
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.data_length, EXPECTED_SIZE);

    ASSERT_EQ(cmd.get_namespace_id(), 0);
    cmd.get_data_ref().result = 1020;
    ASSERT_EQ(cmd.get_namespace_id(), 1020);

    const NamespaceData* nd = reinterpret_cast<const NamespaceData*>(cmd.get_data_ref().cmd.data_pointer.data.address);
...
TEST(NvmeCommandsTest, Identify) {
    ...
    ASSERT_EQ(cmd.get_data_ref().cmd.namespace_id, 456);
    ...
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_controller_data()));
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_namespace_data()));
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_controller_id_list()));
    ASSERT_EQ(cmd.get_data_ref().cmd.data_pointer.data.address, reinterpret_cast<uint64_t>(&cmd.get_namespace_id_list()));
}
