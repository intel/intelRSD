...
TEST(GetPsInputPowerRequest, PackRequestData) {
    request::GetPsInputPower request;
...
TEST(GetPsInputPowerResponse, GetPowerValueWithoutUnpack) {
    response::GetPsInputPower response;
    ASSERT_DOUBLE_EQ(0.0, response.get_power());
}

TEST(GetPsInputPowerResponse, GetPowerValue) {
    response::GetPsInputPower response;

    for (const auto& expected: expected_power_values) {
        const auto& bytes = std::get<0>(expected);
        response.do_unpack(bytes);
        ASSERT_DOUBLE_EQ(std::get<1>(expected), response.get_power());
    }
}
