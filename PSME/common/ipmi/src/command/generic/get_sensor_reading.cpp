...
request::GetSensorReading::GetSensorReading(): Request(uint8_t(NetFn::SENSOR_EVENT),
                                                               uint8_t(Cmd::GET_SENSOR_READING)) {}
request::GetSensorReading::~GetSensorReading() {}

void request::GetSensorReading::pack(vector<uint8_t>& data) const {
    data.push_back(m_sensor_number);
}

response::GetSensorReading::GetSensorReading(): Response(uint8_t(NetFn::SENSOR_EVENT),
                                                         uint8_t(Cmd::GET_SENSOR_READING),
                                                         RESPONSE_SIZE) {}
...
void response::GetSensorReading::unpack(const vector<uint8_t>& data) {
