...
#include "ipmi/command/sdv/get_oob_boot_options.hpp"
...
response::GetOobBootOptions::GetOobBootOptions() :
    Response(sdv::NetFn::RACKSCALE, sdv::Cmd::GET_OOB_BOOT_OPTION, MAX_RESPONSE_SIZE) {}
...
bool response::GetOobBootOptions::valid_basic_parameter(const std::vector<uint8_t>& data) {
    ...
    if (data_size < MIN_RESPONSE_SIZE) {
        throw std::runtime_error(("Cannot unpack OOB " + what() + " Boot Options response. Data length too short."
            " Expected at least: ")
    ...
    if (data_size != v2_payload_size()) {
        throw std::runtime_error(
            ("Cannot unpack OOB " + what() + "Boot Options response. Unexpected data length for v2 packet."
                " Expected: ")
            + std::to_string(v2_payload_size())
            + " Received: "
            + std::to_string(data.size()));
    }
