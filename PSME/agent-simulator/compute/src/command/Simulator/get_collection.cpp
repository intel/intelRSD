...
private:
    ...
    void process_system(const std::string& uuid, const std::string& name,
         ...
         if (is_memory_chunks(name)) {
            process_memory_chunks(uuid, response);
            return;
         }
    ...
    void process_memory_chunks(const std::string& uuid, Response& response) {
        response_add_submodules(ComputeComponents::get_instance()->
    ...
    bool is_memory_chunks(const std::string& name) {
        return name == "MemoryChunks";
    }
...
static Command::Register<GetCollection> g("Simulator");
