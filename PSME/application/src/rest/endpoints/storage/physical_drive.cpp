...
#include "psme/rest/endpoints/storage/physical_drive.hpp"
...
void add_used_by_link(json::Value& json, const server::Request& req,
    ...
    try {
        ...
        link[Common::ODATA_ID] = endpoint::PathBuilder(PathParam::BASE_URL)
                                    ...
                                    .append(id).build();
        json[Common::LINKS][constants::Drive::USED_BY].push_back(std::move(link));
    }
    catch (agent_framework::exceptions::InvalidUuid&) {
        log_error(GET_LOGGER("rest"), "Model PhysicalDrive/UsedBy link error");
        log_error(GET_LOGGER("rest"), "PhysicalDrive " << uuid <<
            " is used by a LogicalDrive (" << parents.front() << ") that does not exist");
    }
}

}

endpoint::PhysicalDrive::PhysicalDrive(const std::string& path) : EndpointBase(path) {}
