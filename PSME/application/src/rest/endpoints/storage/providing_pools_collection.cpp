/*!
 * @brief Providing Pools collection endpoint
 *
 ...
 *
 * @file providing_pools_collection.cpp
 */


#include "psme/rest/constants/constants.hpp"
#include "psme/rest/endpoints/storage/providing_pools_collection.hpp"
...
json::Json make_prototype() {
    ...
    r[Common::NAME] = "ProvidingPools Collection";
    r[Common::DESCRIPTION] = "Collection of Providing Pools";
...
void get_providing_pools(const agent_framework::model::attribute::CapacitySource& capacity_source,
                         json::Json& r) {

    r[Collection::ODATA_COUNT] = static_cast<std::uint32_t>(capacity_source.get_providing_pools().size());
    for (const auto& pool : capacity_source.get_providing_pools()) {
        auto pool_url = psme::rest::endpoint::utils::get_component_url(agent_framework::model::enums::Component::StoragePool,
                                                                       pool);
...
ProvidingPoolsCollection::ProvidingPoolsCollection(const std::string& path) : EndpointBase(path) {}


ProvidingPoolsCollection::~ProvidingPoolsCollection() {}


void ProvidingPoolsCollection::get(const server::Request& request, server::Response& response) {
    ...
    if (storage_pool_uuid.has_value()) {
        ...
        get_providing_pools(capacity_source, r);
