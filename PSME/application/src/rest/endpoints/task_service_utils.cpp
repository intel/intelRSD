...
server::Response call_task_get(const std::string& task_uuid) {
    const std::string& task_url{
        psme::rest::endpoint::utils::get_component_url(agent_framework::model::enums::Component::Task, task_uuid)};
    ...
    get_task_request.params[constants::PathParam::TASK_ID] = std::to_string(
        agent_framework::module::get_manager<agent_framework::model::Task>().get_entry(task_uuid).get_id());
