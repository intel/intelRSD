...
json::Json make_prototype() {
    ...
    response[System::HOSTED_SERVICES][System::STORAGE_SERVICES] = json::Json::value_t::null;
...
void add_dcpmem_erase_action(const server::Request& request, json::Json& response) {
    ...
    response[Common::ACTIONS][Common::OEM][System::HASH_INTEL_OEM_ERASE_OPTANE_DC_PERSISTENT_MEMORY] =
        std::move(dcpmem_erase);
}
...
void fill_storage_service_data(const agent_framework::model::System& system, json::Json& r) {
    ...
    try {
        json::Json storage_services{};
        storage_services[Common::ODATA_ID] = endpoint::PathBuilder(get_component_url(Component::System, system.get_uuid()))
            .append(constants::System::STORAGE_SERVICES).build();
        r[constants::System::HOSTED_SERVICES][constants::System::STORAGE_SERVICES] = storage_services;
    }
    ...
    if (agent_framework::module::get_m2m_manager<agent_framework::model::System, agent_framework::model::StorageService>()
        .parent_exists(system.get_uuid())) {
        // Add role only when there is storage service on the system
...
void fill_performance_configuration_data(const agent_framework::model::System& system, json::Json& r) {
    ...
    r[Common::OEM][Common::RACKSCALE][Common::PERFORMANCE_CONFIGURATION][Common::CURRENT_CONFIGURATION_ID] =
        system.get_current_performance_configuration();
