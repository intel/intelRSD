...
class DeleteEthernetSwitchPort final :
    public network::DeleteEthernetSwitchPort {
public:
    /*! Command constructor */
    DeleteEthernetSwitchPort() { }

    using network::DeleteEthernetSwitchPort::execute;
    ...
    void execute(const Request& request, Response& response) {
        ...
        if(NetworkManager::get_instance()->
            ...
            get_port_manager().remove_entry(uuid);
            response.set_oem(oem);
            message = "Deleted ethernet switch port with uuid: " + uuid;
...
static Command::Register<DeleteEthernetSwitchPort> g("Stubs");
