...
class DeleteEthernetSwitchPortMembers final :
    public network::DeleteEthernetSwitchPortMembers {
public:
    /*! Command constructor */
    DeleteEthernetSwitchPortMembers() { }

    using network::DeleteEthernetSwitchPortMembers::execute;
    ...
    void execute(const Request& request, Response& response) {
        ...
        if(NetworkManager::get_instance()->
            ...
            const auto& sons = NetworkManager::get_instance()->
                get_port_manager().get_keys(uuid);
            message = "For ethernet switch port with uuid: " + uuid + "\n";
            for(const auto& son: sons) {
                if(std::find(members.begin(), members.end(), son) !=
                    members.end()) {
                    NetworkManager::get_instance()->
                        get_port_manager().remove_entry(son);
                    message += "- Deleted port with uuid: " + son + "\n";
                }
            }
            response.set_oem(oem);
            message += "DeleteEthernetSwitchPortMembers finished successfully.";
            log_info(GET_LOGGER("network-agent"), message);
        }
        else {
            message = "No ethernet switch port with uuid: " + uuid;
...
static Command::Register<DeleteEthernetSwitchPortMembers> g("Stubs");
