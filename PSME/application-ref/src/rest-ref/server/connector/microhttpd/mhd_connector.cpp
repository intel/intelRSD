...
#include "psme/rest-ref/server/connector/microhttpd/mhd_connector.hpp"
#include "psme/rest-ref/server/connector/microhttpd/mhd_connector_options.hpp"
extern "C" {
...
using MHDResponsePtr = std::unique_ptr<MHD_Response,
                                       decltype(&MHD_destroy_response)>;

MHDResponsePtr create_response(Response& response) {
    auto body = response.to_buffer();
...
int access_handler_callback(void* cls, struct MHD_Connection *connection,
    ...
    std::unique_ptr<Request, decltype(deleter)>
                request(static_cast<Request*>(*con_cls), deleter);
    ...
    if (!request) {
        ...
        try {
            request->set_method(Method::from_string(method));
        } catch (agent_framework::exceptions::InvalidParameters&) {
            request->set_method(Method::UNKNOWN);
        }
