...
#include "psme/rest-ref/endpoints/acl.hpp"
...
json::Value make_prototype() {
    ...
    r[Common::ODATA_TYPE] =
        "#EthernetSwitchACL.1.0.0.EthernetSwitchACL";
    ...
    json::Value actions;
    std::string hash_bind_acl = "#" + std::string(constants::Acl::BIND_ACL);
    actions[hash_bind_acl][constants::Acl::TARGET] = json::Value::Type::NIL;
    actions[hash_bind_acl][constants::Acl::PORT_ALLOWABLE_VALUES] =
        json::Value::Type::ARRAY;

    std::string hash_unbind_acl =
        "#" + std::string(constants::Acl::UNBIND_ACL);
    actions[hash_unbind_acl][constants::Acl::TARGET] = json::Value::Type::NIL;
    actions[hash_unbind_acl][constants::Acl::PORT_ALLOWABLE_VALUES] =
        json::Value::Type::ARRAY;
...
void add_bound_ports_links(json::Value& json, const std::string& acl_uuid) {
    const auto bound_ports = NetworkManager::get_instance()->
    ...
    for (const auto& bound_port : bound_ports) {
        try {
            ...
            link[Common::ODATA_ID] = endpoint::utils::get_component_url(
                enums::Component::SwitchPort, bound_port);
...
endpoint::Acl::Acl(const std::string& path) : Endpoint(path) {}
endpoint::Acl::~Acl() {}
...
void endpoint::Acl::get(const server::Request& req, server::Response& res) {
    ...
    const auto acl = psme::rest::model::Find<agent_framework::model::Acl>(
    ...
    auto is_not_bound_predicate =
        [&acl] (const agent_framework::model::SwitchPort& port) {
            return false == NetworkManager::get_instance()->
                get_acl_port_manager().entry_exists(acl.get_uuid(),
                port.get_uuid());
        };

    const auto unbound_ports = NetworkManager::get_instance()->
    ...
    for (const auto& unbound_port : unbound_ports) {
        json::Value link;
        link[Common::ODATA_ID] = endpoint::utils::get_component_url(
            enums::Component::SwitchPort, unbound_port);
        r[constants::Common::ACTIONS][hash_bind_acl]
    ...
    std::string hash_unbind_acl =
        "#" + std::string(constants::Acl::UNBIND_ACL);
    r[constants::Common::ACTIONS][hash_unbind_acl]
     [constants::Acl::TARGET] =
        PathBuilder(req)
        .append(constants::Common::ACTIONS)
        .append(constants::Acl::UNBIND_ACL).build();
...
void endpoint::Acl::del(const server::Request& req, server::Response& res) {
    auto acl = psme::rest::model::Find
        <agent_framework::model::Acl>(req.params[PathParam::ACL_ID]).via
