...
#include "psme/rest-ref/endpoints/logical_drive.hpp"
...
json::Value make_prototype() {
    ...
    r[Common::ODATA_TYPE] = "#LogicalDrive.1.0.0.LogicalDrive";
...
void add_targets_link(json::Value& json, const server::Request& req,
    ...
    for (const auto& target_id : targets_id) {
        ...
        for (const auto& lun : luns) {
            ...
            json[Common::LINKS][constants::LogicalDrive::TARGETS].push_back(
                std::move(link));
        }
    }
}
...
void add_physical_drives_link(json::Value& json, const server::Request& req,
    ...
    for (const auto& phys_member : phys_members) {
        try {
            ...
            auto phys_id = agent_framework::module::StorageManager::get_instance()
...
void add_logical_drives_link(json::Value& json, const server::Request& req,
    ...
    for (const auto& log_member : log_members) {
        try {
            // convert children UUID into ID
            auto log_id = agent_framework::module::StorageManager::get_instance()
            ...
            link[Common::ODATA_ID] =
                endpoint::PathBuilder(PathParam::BASE_URL)
                            ...
                            .append(constants::StorageService::LOGICAL_DRIVES)
                            .append(log_id).build();
            json[Common::LINKS][constants::LogicalDrive::LOGICAL_DRIVES].push_back(
            std::move(link));
        }
        catch (agent_framework::exceptions::InvalidParameters&) {
...
endpoint::LogicalDrive::LogicalDrive(const std::string& path) : Endpoint(path) {}
...
void endpoint::LogicalDrive::get(const server::Request& req, server::Response& res) {
    ...
    r[Common::STATUS][Common::HEALTH_ROLLUP] = endpoint::utils::rollup_logical_drive_health(drive.get_uuid()).to_string();
    ...
    if ( drive.get_image().has_value() && !drive.get_image().value().empty()) {
        r[constants::LogicalDrive::IMAGE] = drive.get_image();
    } else {
        r[constants::LogicalDrive::IMAGE] = json::Value::Type::NIL;
    }

    r[constants::LogicalDrive::BOOTABLE] = drive.get_bootable();
    r[constants::LogicalDrive::SNAPSHOT] = drive.get_snapshot();
...
void endpoint::LogicalDrive::del(const server::Request& req, server::Response& res)
{
    ...
    auto gami_req = agent_framework::model::requests::DeleteLogicalDrive(
        drive->get_uuid());

    // try removing port from agent's model
    AgentManager::get_instance()->
        call_method<agent_framework::model::responses::DeleteLogicalDrive>(
            drive->get_agent_id(), gami_req);

    // remove port from REST model, DO NOT use drive reference after this line!
