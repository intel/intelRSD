...
#include "psme/rest-ref/endpoints/devices_collection.hpp"
...
json::Value make_prototype() {
    ...
    r[Common::ODATA_CONTEXT] = "/redfish/v1/$metadata#Systems/Members/__SYSTEM_ID__/Adapters/Members/__ADAPTER_ID__/Devices";
    r[Common::ODATA_ID] = json::Value::Type::NIL;
    r[Common::ODATA_TYPE] = "#DeviceCollection.DeviceCollection";
    r[Common::NAME] = "Devices Collection";
    r[Common::DESCRIPTION] = "Collection of Devices";
...
DevicesCollection::DevicesCollection(const std::string& path) : Endpoint(path) {}
DevicesCollection::~DevicesCollection() {}

void DevicesCollection::get(const server::Request& req, server::Response& res) {
    using psme::rest::model::Find;
    using agent_framework::model::StorageController;
    ...
    auto system_id = psme::rest::model::Find<agent_framework::model::System>(req.params[PathParam::SYSTEM_ID])
        .get_one()->get_id();
    auto adapter = Find<StorageController>(req.params[PathParam::ADAPTER_ID])
    ...
    json[Common::ODATA_CONTEXT] = std::regex_replace(json[Common::ODATA_CONTEXT].as_string(),
                                                     std::regex("__ADAPTER_ID__"), std::to_string(adapter->get_id()));

    auto keys = agent_framework::module::ComputeManager::get_instance()->
                        get_drive_manager().get_ids(adapter->get_uuid());
