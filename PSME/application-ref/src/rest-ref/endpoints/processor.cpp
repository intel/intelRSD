...
#include "psme/rest-ref/endpoints/processor.hpp"
...
json::Value make_prototype() {
    ...
    r[Common::ODATA_TYPE] = "#Processor.1.0.0.Processor";
    ...
    rs[Common::CONTAINED_BY] = json::Value::Type::NIL;
...
endpoint::Processor::Processor(const std::string& path) : Endpoint(path) {}
...
void endpoint::Processor::get(const server::Request& req, server::Response& res) {
    ...
    endpoint::utils::status_to_json(processor,json);
    ...
    json[constants::Processor::PROC_ID][Common::VENDOR_ID] =
        cpuid.get_vendor_id();
    json[constants::Processor::PROC_ID][constants::Processor::IDENTIFICATION_REGISTERS] =
        cpuid.get_numeric_id();
    json[constants::Processor::PROC_ID][constants::Processor::EFFECTIVE_FAMILY] =
        cpuid.get_family();
    json[constants::Processor::PROC_ID][constants::Processor::EFFECTIVE_MODEL] =
        cpuid.get_model();
    json[constants::Processor::PROC_ID][constants::Processor::STEP] =
        cpuid.get_step();
    json[constants::Processor::PROC_ID][constants::Processor::MICROCODE_INFO] =
        cpuid.get_microcode_info();
