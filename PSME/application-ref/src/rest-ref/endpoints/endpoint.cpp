...
void psme::rest::server::http_method_not_allowed(const Request&, Response& response) {
    auto error = error::ErrorFactory::create_method_not_allowed_error();
...
void Endpoint::update_modified_time() {
    m_modified_time = ::get_current_time();
}

void Endpoint::get(const Request& request, Response& response) {
    http_method_not_allowed(request, response);
}

void Endpoint::del(const Request& request, Response& response) {
    http_method_not_allowed(request, response);
}

void Endpoint::post(const Request& request, Response& response) {
    http_method_not_allowed(request, response);
}

void Endpoint::patch(const Request& request, Response& response) {
    http_method_not_allowed(request, response);
}

void Endpoint::put(const Request& request, Response& response) {
    http_method_not_allowed(request, response);
}
