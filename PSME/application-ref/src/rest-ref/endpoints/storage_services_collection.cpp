...
#include "psme/rest-ref/endpoints/storage_services_collection.hpp"
...
StorageServiceCollection::StorageServiceCollection(const std::string& path) : Endpoint(path) {}
...
void StorageServiceCollection::get(const server::Request& req, server::Response& res) {
    ...
    auto service_ids = agent_framework::module::StorageManager::get_instance() -> get_storage_services_manager().get_ids();
