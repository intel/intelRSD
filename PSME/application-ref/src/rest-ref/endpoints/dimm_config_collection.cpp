...
#include "psme/rest-ref/endpoints/dimm_config_collection.hpp"
#include <regex>
...
json::Value make_prototype() {
    ...
    r[Common::NAME] = "Memory Configuration Collection";
    r[Common::DESCRIPTION] = "Collection of Memory Configs";
...
DimmConfigCollection::DimmConfigCollection(const std::string& path) : Endpoint(path) {}
DimmConfigCollection::~DimmConfigCollection() {}

void DimmConfigCollection::get(const server::Request& req, server::Response&
    res) {
    ...
    auto keys = agent_framework::module::ComputeManager::get_instance()->
                        get_dimm_manager().get_ids(system_uuid);
