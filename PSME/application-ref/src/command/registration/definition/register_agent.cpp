...
namespace {
    ...
    std::string capabilities_to_string(
                            const psme::core::agent::Capabilities& caps) {
...
public:
    ...
    void execute(const Request& request, Response& response) {
        ...
        log_info(GET_LOGGER("registration"),
                "Agent " << request.get_gami_id() << " registered.");
    }
...
private:
    std::string register_agent(const Request& request) {
        ...
        log_info(GET_LOGGER("rest"),
                "Registration. request.ip=" << request.get_ipv4address()
        ...
        auto agent = std::make_shared<psme::core::agent::JsonAgent>(
                request.get_gami_id(),
                request.get_ipv4address(),
                request.get_port());
