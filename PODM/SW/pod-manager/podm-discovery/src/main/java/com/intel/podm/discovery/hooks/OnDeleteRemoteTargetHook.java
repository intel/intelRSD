...
public class OnDeleteRemoteTargetHook implements OnDeleteHook {
    @Inject
    private GenericDao genericDao;

    @Override
    public void execute(DomainObject domainObject) {
        RemoteTarget remoteTarget = (RemoteTarget) domainObject;
        ComposedNode composedNode = remoteTarget.getComposedNode();

        if (composedNode != null) {
            composedNode.setStatus(OFFLINE_CRITICAL_STATUS);
        }

        remoteTarget.getRemoteTargetIscsiAddresses().forEach(genericDao::remove);
    }

    @Override
    public boolean canExecute(DomainObject domainObject) {
        return domainObject instanceof RemoteTarget;
    }
}
