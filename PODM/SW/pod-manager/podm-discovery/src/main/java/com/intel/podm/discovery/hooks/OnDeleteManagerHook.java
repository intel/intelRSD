...
public class OnDeleteManagerHook implements OnDeleteHook {
    ...
    public void execute(DomainObject domainObject) {
        ...
        removeConsoles(manager);
    }

    @Override
    public boolean canExecute(DomainObject domainObject) {
        return domainObject instanceof Manager;
    }

    private void removeConsoles(Manager manager) {
        deleteConsole(manager.getGraphicalConsole());
        deleteConsole(manager.getCommandShell());
        deleteConsole(manager.getSerialConsole());
    }

    private void deleteConsole(Console console) {
        if (console != null) {
            genericDao.remove(console);
        }
    }

    private void deleteConsole(GraphicalConsole console) {
        if (console != null) {
            genericDao.remove(console);
        }
    }
