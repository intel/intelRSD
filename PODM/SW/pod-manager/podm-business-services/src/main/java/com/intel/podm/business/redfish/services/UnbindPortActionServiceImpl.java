...
@RequestScoped
public class UnbindPortActionServiceImpl implements ActionService<UnbindPortActionRequest> {
    @Inject
    ...
    @Override
    @Transactional(NEVER)
    public void perform(Context aclContext, UnbindPortActionRequest request) throws BusinessApiException, TimeoutException {
        taskCoordinator.run(serviceTraverser.traverseServiceUuid(aclContext),
            () -> aclPortActionsService.unbindPort(aclContext, request.getPortContext()));
    }
}
