...
class EthernetSwitchPortCreationServiceImpl implements CreationService<RedfishEthernetSwitchPort> {
    ...
    @Override
    @Transactional(NEVER)
    public Context create(Context creationalContext, RedfishEthernetSwitchPort representation) throws BusinessApiException, TimeoutException {
        return taskCoordinator.call(
            traverser.traverseServiceUuid(creationalContext),
            () -> actionService.createSwitchPort(creationalContext, representation)
        );
    }
}
