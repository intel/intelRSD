...
@RequestScoped
public class EthernetSwitchStaticMacRemovalServiceImpl implements RemovalService<EthernetSwitchStaticMacDto> {
    @Inject
    ...
    public void perform(Context target) throws BusinessApiException, TimeoutException {
            taskCoordinator.run(traverser.traverseServiceUuid(target), () -> actionsService.deleteStaticMac(target));
    }
}
