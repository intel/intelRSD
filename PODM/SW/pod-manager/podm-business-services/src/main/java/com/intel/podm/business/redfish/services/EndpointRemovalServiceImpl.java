...
@RequestScoped
public class EndpointRemovalServiceImpl implements RemovalService<EndpointDto> {
    @Inject
    ...
    @Inject
    private EndpointActionService actionsService;

    @Override
    public void perform(Context target) throws BusinessApiException, TimeoutException {
        taskCoordinator.run(traverser.traverseServiceUuid(target), () -> actionsService.deleteEndpoint(target));
    }
}
