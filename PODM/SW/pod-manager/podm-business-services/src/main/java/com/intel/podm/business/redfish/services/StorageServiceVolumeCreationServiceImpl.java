...
@RequestScoped
public class StorageServiceVolumeCreationServiceImpl implements CreationService<StorageServiceVolume> {
    ...
    @Override
    @Transactional(NEVER)
    public Context create(Context creationalContext, StorageServiceVolume representation) throws BusinessApiException, TimeoutException {
        return taskCoordinator.call(
            traverser.traverseServiceUuid(creationalContext),
            () -> volumeActionsService.create(creationalContext, representation)
        );
    }
}
