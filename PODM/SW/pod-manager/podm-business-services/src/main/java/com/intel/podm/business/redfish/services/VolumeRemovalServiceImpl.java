...
@RequestScoped
public class VolumeRemovalServiceImpl implements RemovalService<VolumeDto> {
    @Inject
    ...
    @Inject
    private VolumeActionsService actionsService;

    @Override
    public void perform(Context target) throws BusinessApiException, TimeoutException {
        taskCoordinator.run(traverser.traverseServiceUuid(target), () -> actionsService.deleteVolume(target));
    }
}
