...
@RequestScoped
public class ZoneRemovalServiceImpl implements RemovalService<ZoneDto> {
    @Inject
    ...
    @Inject
    private ZoneActionsService actionsService;

    @Override
    public void perform(Context target) throws BusinessApiException, TimeoutException {
        taskCoordinator.run(traverser.traverseServiceUuid(target), () -> actionsService.deleteZone(target));
    }
}
