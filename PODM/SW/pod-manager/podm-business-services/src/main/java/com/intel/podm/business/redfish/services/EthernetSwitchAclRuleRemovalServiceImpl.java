...
@RequestScoped
public class EthernetSwitchAclRuleRemovalServiceImpl implements RemovalService<RedfishEthernetSwitchAclRule> {
    @Inject
    ...
    @Inject
    private EthernetSwitchAclRuleActionsService actionsService;

    @Override
    public void perform(Context target) throws BusinessApiException, TimeoutException {
        taskCoordinator.run(traverser.traverseServiceUuid(target), () -> actionsService.deleteAclRule(target));
    }
}
