...
import com.intel.podm.business.redfish.services.assembly.tasks.DeallocateRemoteTarget;
import com.intel.podm.business.redfish.services.assembly.tasks.DetachPcieDriveTask;
import com.intel.podm.business.redfish.services.assembly.tasks.LogicalDriveCleanUpTask;
...
public class NodeDisassembler {
    ...
    public Collection<NodeTask> getDisassemblyTasks(Id composedNodeId) throws ContextResolvingException {
        ...
        tasks.addAll(prepareLogicalVolumeCleanUpTask(composedNode));
    ...
    private Collection<? extends NodeTask> prepareLogicalVolumeCleanUpTask(ComposedNode composedNode) {
        return composedNode.getLogicalDrives()
            .stream()
            .map(logicalDrive -> beanFactory.create(LogicalDriveCleanUpTask.class)
                .setLogicalDrive(logicalDrive.getId()))
            .collect(toList());
    }
    ...
    private Collection<? extends NodeRemovalTask> prepareComputerSystemDeallocationTasks(ComposedNode composedNode) {
    ...
    private Collection<? extends NodeRemovalTask> prepareRemoteTargetDeallocationTasks(ComposedNode composedNode) {
