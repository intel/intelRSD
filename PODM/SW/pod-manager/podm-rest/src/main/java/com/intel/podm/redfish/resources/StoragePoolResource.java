...
public class StoragePoolResource extends BaseResource {
    @Inject
    private ReaderService<StoragePoolDto> readerService;

    @GET
    @Override
    public RedfishResourceAmazingWrapper get() {
        Context context = getCurrentContext();
        StoragePoolDto storagePoolDto = getOrThrow(() -> readerService.getResource(context));
        return new RedfishResourceAmazingWrapper(context, storagePoolDto);
    }
    ...
    @Path(ALLOCATED_VOLUMES_RESOURCE_NAME)
    public VolumeCollectionResource getAllocatedVolumes() {
        return getResource(VolumeCollectionResource.class);
    }

    @Path(ALLOCATED_POOLS_RESOURCE_NAME)
    public StoragePoolCollectionResource getAllocatedPools() {
