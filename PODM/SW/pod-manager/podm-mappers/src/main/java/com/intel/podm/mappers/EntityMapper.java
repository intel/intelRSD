...
public abstract class EntityMapper<S extends ExternalServiceResource, T extends Entity> extends BrilliantMapper<S, T> {
    protected S source;
    protected T target;
    ...
    protected void performNotAutomatedMapping(S source, T target) {
        ...
        if (target instanceof DiscoverableEntity) {
            DiscoverableEntity discoverableEntity = (DiscoverableEntity) target;
            unknownOemMapper.map(source.getUnknownOems(), discoverableEntity.getUnknownOems(), discoverableEntity::addUnknownOem);
        }
    }
