...
public class UrlProvider {
    ...
    private URI getUriFromTmpLeasesRecord(TmpLeasesRecord record) {
        ...
        boolean sslEnabled = config.isSslEnabledForServicesOfType(record.getServiceType());
        int port = sslEnabled
                ? config.getSslPortForServicesOfType(serviceType)
                : config.getDefaultPortForServicesOfType(serviceType);
