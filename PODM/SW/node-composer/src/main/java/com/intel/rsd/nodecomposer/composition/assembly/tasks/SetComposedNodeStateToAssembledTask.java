...
public class SetComposedNodeStateToAssembledTask extends NodeTask implements Serializable {
    private static final long serialVersionUID = 5845933647614194496L;

    @Autowired
    private transient ComposedNodeStateChanger composedNodeStateChanger;

    @Override
    @TimeMeasured(tag = "[AssemblyTask]")
    public void run() {
        composedNodeStateChanger.change(composedNodeODataId, ASSEMBLED);
    }
}
