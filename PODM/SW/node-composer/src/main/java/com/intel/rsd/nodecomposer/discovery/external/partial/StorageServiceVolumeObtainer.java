...
public class StorageServiceVolumeObtainer {
    ...
    @Autowired
    public StorageServiceVolumeObtainer(WebClientBuilder webClientBuilder, EntityMultiMapper entityMultiMapper) {
    ...
    public Volume discoverStorageServiceVolume(StorageService storageService, URI volumeUri) throws WebClientRequestException {
        ...
        try (WebClient webClient = webClientBuilder.createResourceManagerInstance().retryable().build()) {
            ...
            Volume volume = (Volume) entityMultiMapper.map(volumeResource);
    ...
    private void mapCapacitySources(Volume volume, Iterable<ResourceSupplier> capacitySources, StorageService storageService)
        ...
        for (ResourceSupplier capacitySourceResourceSupplier : capacitySources) {
            ...
            CapacitySource capacitySource = (CapacitySource) entityMultiMapper.map(capacitySourcesResource);
            ...
            for (ResourceSupplier storagePoolResourceSupplier : capacitySourcesResource.getProvidingPools()) {
                ...
                StoragePool storagePool = (StoragePool) entityMultiMapper.map(storagePoolResource);
                ...
                Set<Volume> currentlyAllocatedVolumes = new HashSet<>(getAllocatedVolumes(storagePoolResource, storageService));
    ...
    private Set<Volume> getAllocatedVolumes(StoragePoolResource storagePoolResource, StorageService storageService)
        throws WebClientRequestException {
