...
LEASES_FILE = '/tmp/leases'
LOG_FILE = '/tmp/parse.leases.txt'

def parse_arguments():
    parser = argparse.ArgumentParser("parse.leases.py")
...
def on_expiry(arguments, logger):
    ...
    predicate = lambda lease : lease['ip'] != arguments.ip and lease['hostname'] != arguments.hostname
...
def get_leases():
    ...
    with open(LEASES_FILE, 'a+') as leases:
...
def create_logger():
    ...
    logger = logging.getLogger('parse.leases')
